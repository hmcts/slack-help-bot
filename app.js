    ...
    async ({ body, action, ack, client, context }) => {
        try {
            ...
            if (!helpRequest.summary) {
                ...
                errorMessage =
                    "Please specify the team experiencing the problem.";
            ...
            const jiraId = await createHelpRequest({
                ...
                labels: [
                    `area-${helpRequest.area.value
                        .toLowerCase()
                        .replace(" ", "-")}`,
                    `team-${helpRequest.team.value
                        .toLowerCase()
                        .replace(" ", "-")}`,
                ],
            });
            ...
            console.error(
                "An error occurred when submitting a help form: " + error,
            );
        }
    },
);
    ...
    try {
        ...
        if (event.channel === reportChannelId && event.thread_ts) {
            ...
            if (
                ...
                helpRequestMessages[0].text ===
                    "New platform help request raised"
            ) {
...
\`duplicate\ [JiraID]\` - Marks this ticket as a duplicate of the specified ID`;
                    ...
                    if (result) {
                        ...
                        const issueDescription = await getIssueDescription(
                            parentJiraId,
                        );
                        ...
                        const parentSlackUrl =
                            extractSlackLinkFromText(issueDescription);
                        const currentIssueJiraId =
                            extractJiraIdFromBlocks(blocks);
...
app.view("document_help_request", async ({ ack, body, view, client }) => {
    try {
        ...
        const documentation = {
            category:
                body.view.state.values.category_block.category.selected_option
                    .value,
    ...
    try {
        ...
        if (event.channel === reportChannelId && event.thread_ts) {
            ...
            if (
                ...
                const jiraId = extractJiraIdFromBlocks(
                    helpRequestMessages[0].blocks,
                );
    ...
    async ({ body, action, ack, client, context }) => {
        try {
            ...
            const parsedPromises = issues.flatMap(async (result) => {
                ...
                return appHomeIssueBlocks({
                    summary: result.fields.summary,
                    slackLink: extractSlackLinkFromText(
